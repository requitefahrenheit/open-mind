...
TYPE_SCHEMAS = {
    ...
    "appointment": {
        ...
        "appt_status": {"type": "select", "label": "Status", "options": ["upcoming", "completed", "cancelled"]},
    ...
    "daily": {},
    "chore": {
        "task_status": {"type": "select", "label": "Status", "options": ["todo", "in-progress", "done"]},
        "priority": {"type": "select", "label": "Priority", "options": ["low", "medium", "high"]},
        "depends_on": {"type": "text", "label": "Depends On"},
    },
    "diary": {
        "mood": {"type": "select", "label": "Mood", "options": ["great", "good", "okay", "rough", "bad"]},
        "location": {"type": "text", "label": "Location"},
    },
}
...

{
  ...
  "node_type": "idea" | "url" | "paper" | "project" | "appointment" | "note" | "image" | "task" | "chore" | "diary",
...
- "chore" — actionable tasks, errands, things to buy/fix/do (buy milk, fix faucet, call dentist, clean garage)
- "diary" — personal reflections, feelings, experiences, what happened today, journal entries
- "appointment" — scheduled events with a specific date/time (meeting tuesday, dentist 3pm, dinner friday)
- "idea" — concepts, what-if thoughts, creative ideas, hypotheses, things to explore
- "task" — work/project tasks with more structure than chores
- "note" — general information capture that doesn't fit the above

Response text by type:
- chore: brief confirmation, mention how many open chores exist if possible ("Added. You have N open chores.")
- diary: warm but minimal ("Noted." or "Saved to your journal.")
- appointment: confirm the parsed date/time back ("Got it: [label], [date] @ [time]")
- idea: encouraging, mention if related ideas exist ("Captured! Relates to [topic].")
- note/other: simple confirmation
"""
...
async def api_sms(request: Request):
        ...
        for i in range(num_media):
            ...
            try:
                ...
                if is_image:
                    ...
                    # Build generated content (caption/description/OCR)
                    generated_parts = []
                    if caption:
                    ...
                    # User's SMS text = user description on every image
                    if body and generated:
                        img_content = f"{body}\n\n{generated}"
                    elif body:
                        img_content = body
                    elif generated:
                    ...
                    create_node(content=img_content, node_type="image", image_path=fname,
                                label=caption[:60] if caption else (body[:60] if body else None),
                                metadata=meta if meta else None)
                    media_saved += 1
                    response_text += f"Image {media_saved} saved. "
                    ...
                    create_node(content=pdf_content, node_type="paper",
                                ...
                                metadata=meta)
                    media_saved += 1
                    response_text += f"PDF {media_saved} saved. "

            except Exception as e:
                log.error(f"SMS media error: {e}")
        if media_saved > 1:
            response_text = f"{media_saved} items saved with your description. " + response_text
    ...
    if body and num_media == 0:
        ...
        sms_response = data.get("response", "Done.")

        # Enrich response with type-aware context
        try:
            node_type = data.get("type") or data.get("node_type")
            if node_type == "chore":
                open_chores = db.execute("SELECT COUNT(*) FROM nodes WHERE type='chore' AND json_extract(meta, '$.task_status') != 'done'").fetchone()[0]
                sms_response += f" ({open_chores} open chores)"
            elif node_type == "appointment":
                upcoming = db.execute("SELECT COUNT(*) FROM nodes WHERE type='appointment' AND due_date >= datetime('now')").fetchone()[0]
                sms_response += f" ({upcoming} upcoming)"
        except Exception:
            pass
